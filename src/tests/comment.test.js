...
test('check code editor works', async () => {
  ...
  const toolbar = document.getElementsByClassName('ck-toolbar__items');

  await waitFor(() => {
    expect(toolbar[0].lastChild).toBeInTheDocument();
  });
  
  expect(toolbar[0].lastChild).toBeInTheDocument();

  let form = document.getElementsByClassName('ck-balloon-panel_visible')[0];

  expect(form).toBe(undefined);

  fireEvent.click(toolbar[0].lastChild);

  form = document.getElementsByClassName('ck-balloon-panel_visible')[0];
